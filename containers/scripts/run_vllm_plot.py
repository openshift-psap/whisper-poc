...
def process_files(file_prefix):
    ...
    for batch_size in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]:
        json_filename = f"{output_dir}/output-{file_prefix}-{batch_size:03d}.json"
    ...
    if not found_files:
        print(f"No data found for file prefix '{file_prefix}'. Skipping...")
        return
    ...
    for metric in metrics:
        ...
        fig.write_image(f"{images_dir}/{file_prefix}-{metric}.png", scale=3)
